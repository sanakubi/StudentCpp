...
int GetXYone(){
    ...
    size_x = sizeof(x) / sizeof(x[0]);
    for (int i = 0; i < size_x; i++) {
        xy_fin += pow(x[i], 2) * log(y[i]);
    }
    return xy_fin;
}

void GetB(){
    b = ( N * GetXYone() - (GetYone() * GetXone()) ) / ( N * GetXtwo() - pow( GetXone(), 2) );

}
void GetA(){
    a = (1 / N) * GetYone() - (b / N) * GetXone();
